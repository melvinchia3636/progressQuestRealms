...
class HallOfInfamy(QWidget):
    def __init__(self, realm):
        super(HallOfInfamy, self).__init__()
    ...
    def scrapeData(self, query=None):
        raw = requests.get("http://progressquest.com/{}.php?cheaters=1&{}".format(self.realm.lower(), f"name={query}" if query else f"min={(self.page-1)*10+1}&max=10")).text
        soup = bs(raw, 'lxml')
    ...
    def setupUI(self):
        self.setObjectName("HallOfInfamy")

        self.hallOfInfamyLayout = QVBoxLayout(self)
        self.hallOfInfamyLayout.setObjectName("outerWrapper")
        ...
        self.hallOfInfamyTable = QTableWidget(self)

        self.hallOfInfamyTable.setObjectName("hallOfInfamyTable")
        self.hallOfInfamyTable.setColumnCount(len(self.data[0]))
        self.hallOfInfamyTable.setRowCount(len(self.data))
        self.hallOfInfamyTable.setHorizontalHeaderLabels([
            "Rank",
            "Name",
            "Race",
            "Class",
        ...
        self.hallOfInfamyTable.verticalHeader().setVisible(False)
        self.hallOfInfamyTable.setEditTriggers(QTableWidget.NoEditTriggers)
        self.hallOfInfamyTable.setSelectionBehavior(QTableWidget.SelectRows)
        self.hallOfInfamyTable.setSelectionMode(QTableWidget.SingleSelection)
        self.hallOfInfamyTable.setSortingEnabled(True)

        header = self.hallOfInfamyTable.horizontalHeader()
        ...
        self.hallOfInfamyLayout.addLayout(self.topWrapper)
        self.hallOfInfamyLayout.addWidget(self.hallOfInfamyTable)
        self.hallOfInfamyLayout.addLayout(self.bottomWrapper)
    ...
    def updateTable(self):
        self.hallOfInfamyTable.setRowCount(len(self.data))
        self.hallOfInfamyTable.setSpan(0, 0, 1, 1)
        ...
        try:
            for i, v in enumerate(self.data):
                if v[1].lower() == self.queryInput.text().lower():
                    self.hallOfInfamyTable.selectRow(i)
    ...
    def submitSearch(self):
        ...
        if all([len(i) < 5 for i in self.data]):
            self.hallOfInfamyTable.setRowCount(1)
            self.hallOfInfamyTable.setItem(0, 0, QTableWidgetItem("No results found."))
            self.hallOfInfamyTable.setSpan(0, 0, 1, 11)
            return
